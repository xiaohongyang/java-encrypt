...
public class DataCertCreate {
    private String path = "H:/tmp/ssl01/";
    ...
    public X509Certificate generateCert(String[] info, KeyPair keyPair_root, KeyPair keyPair_user)
        ...
        certGen.setIssuerDN(new X509Name("CN="+info[0]+", OU=hnu, O=university , C=china"));
        certGen.setNotBefore(new Date(Long.parseLong(info[6])));
        certGen.setNotAfter(new Date(Long.parseLong(info[7])));
        certGen.setSubjectDN(new X509Name("CN=" +info[0]+ ", "+"C=" + info[9] + ",OU=" + info[1] + ",O=" + info[2] + ",C=" + info[3] + ",L="
                + info[4] + ",ST=" + info[3]));
     ...
     * 私钥方法
     */
    private String KEYSTORE_PASSWORD = "20230412";
    ...
    public void generateJKS(String[] info) {
        try {
            ...
            keyStore.store(new FileOutputStream(path + info[0] + ".jks"), KEYSTORE_PASSWORD.toCharArray());
    ...
    public void storeJKS(String[] info, KeyPair keyPair_root, KeyPair keyPair_user) {
        ...
        try {
            ...
            keyStore.load(new FileInputStream(path + info[0] + ".jks"), KEYSTORE_PASSWORD.toCharArray());
    ...
    public Boolean toPFX(String[] info) {
        try {
            String pfx_keystore_file = path + info[0] + ".pfx";
            String jkx_keystore_file = path + info[0] + ".jks";
    ...
    public static void main(String[] args) throws NoSuchAlgorithmException, InvalidKeyException,
        ...
        System.out.printf((new Date().getTime() + (long) 3600*24*365*3*1000)  + "<<<<<<<");
        String[] info = { "roche-test.wshoto.com", "hnu", "university", "jiangsu", "wuxi", "changsha", new Date().getTime() + "", (new Date().getTime() + (long)3600*24*365*3*1000) + "", "19987827431424231121998782743142423112","CN" };
        // 生成公钥
